...
// Funções auxiliares ficam fora do objeto principal
...
const partyController = {
    create: async(req, res) => {
        try {
            ...
            if(party.services && !checkPartyBudget(party.budget, party.services)) {
                res.status(406).json({msg: "O seu orçammento é insuficiente."})
                return
            }
    ...
    update: async(req,res) => {
        try {
            ...
            res.status(200).json({party, mgs: "Festa atualizada com sucesso!"})
        } catch (error) {
            console.log(error)
        }
    }
